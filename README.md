...
`VERSION` is the current git tag without a leading `v`, or `1.2.1` if untagged. The release workflow sets it from the `v*` tag. The script then runs `--smoke` on the AppImage and fails if that does not print `SMOKE=ok`.
