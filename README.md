...
`VERSION` is the current git tag without a leading `v`, or `1.1.2` if untagged. The release workflow sets it from the `v*` tag. The script then runs `--smoke` on the AppImage and fails if that does not print `SMOKE=ok`.
...
Sidebar: Overview, Leftovers, Stale Apps, Outdated, Packages, Settings. The last scan is shown immediately if one was saved. AppAttic then checks in the background whether that scan is still current, and rescans only if apps, brew, leftover folders, or the 24-hour age limit changed. Checkmarks on leftovers, stale apps, outdated rows, and Packages rows survive a background refresh if those items are still present. Settings (confirm before running; on macOS, include system apps) and ignored leftover paths persist in `settings.json` (see above). The Linux window does not scan installed system apps. Ignore a leftover from its inspector to hide it on later scans. Include-in-cleanup uses the inspector toggle and toolbar Select All.
